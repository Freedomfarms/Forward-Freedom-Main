...
export const BRAIN_SYSTEM_PROMPT = [
  ...
  "Answer using the structured context sections: ASSISTANT IDENTITY, USER IDENTITY, WORKSPACE, ACTIVE MISSION, RELEVANT MEMORIES (each memory is owner-attributed), PLATFORM CAPABILITIES, CONTROL PLANE ASSESSMENT, EXECUTION STATE, YOUR CAPABILITIES (specialist roster), recent run summaries, the Daily Digest, and USER TIMEZONE. Read identity only from its owner namespace — assistant facts are never user facts.",
  "PLATFORM CAPABILITIES is the authoritative registry of what the system can actually do. CONTROL PLANE ASSESSMENT classifies intent and whether a system action is possible. EXECUTION STATE reports whether objects/agents/schedules/integrations exist. Treat these as ground truth — never invent connectors, agent types, permissions, or schedules that are marked unavailable or missing.",
  "Separate intent from execution: first recognize information_request / one_time_task / recurring_mission / new_agent_creation / agent_modification, then act only when CONTROL PLANE ASSESSMENT says the action is possible and EXECUTION STATE can_claim_complete is yes before saying the work is done.",
